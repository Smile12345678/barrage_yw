...
(function (main) {
    ...
    function update_proto(path_s) {
        ...
        let paths = `${storage_o.cache_a.output_path_s}/${storage_o.cache_a.output_name_s}.js`.replace(/\\/g, "/");
        let storage_path_s = `${storage_o.cache_a.storage_path_s}/*.proto`.replace(/\\/g, "/");
        // ------------------生成js
        child_process.exec("pbjs -t static-module -w commonjs -o " + paths + " " + storage_path_s, {
            cwd: temp1_s,
            maxBuffer: 5000 * 1024,
            ...
            if (storage_o.cache_a.replace_text1_s && storage_o.cache_a.replace_text2_s) {
                fs.readFile(paths, 'utf-8', (err_a, data_s) => {
                    if (err_a) {
                    ...
                    fs.writeFile(paths, data_s, (err, data) => {
                        if (err) {
