            ...
            methods: {
                ...
                _onAnalyzeExcelDirPath(dir) {
                    ...
                    if (dir) {
                        ...
                        // this._addLog("目录不存在：" + JSON.stringify(excelData[0]));
                ...
                _getJavaScriptSaveData(excelData, itemSheet) {
                    ...
                    let sheetFormatData = [];
                    for (let i = 4; i < excelData.length; i++) { // i 初始行数
                        ...
                        for (let j = 0; j < title.length; j++) {
                            ...
                            // this._addLog("目录不存在：" + key + value);
                            saveLineData[key] = value;
                        }
                        // this._addLog("目录不存在：" + JSON.stringify(saveLineData));
                        // sheetFormatData[lineData[0].toString()] = saveLineData;
                        
                        sheetFormatData.push(saveLineData);
                    }                    
                        ...
                        if (itemSheet.isUse) {
                            ...
                            if (sheetData) {
                                if (sheetData.length > 2) {
                                    ...
                                    this.saveFile(itemSheet.sheet.toLowerCase(),sheetJsData);
                                    // 检测重复问题
                                    if (jsSaveData[itemSheet.sheet] === undefined) {
                                        // jsSaveData[itemSheet.sheet] = sheetJsData;
                                        jsSaveDataOne = sheetJsData;
                                    } else {
                ...
                saveFile(jsFileName, jsSaveDataOne) {
                    // =====================>>>>  保存json文件   <<<=================================
                    if (this.isMergeJson) {
                        let saveFileFullPath = path.join(this.jsonSavePath, this.jsonAllCfgFileName + ".json");
                    ...
                    // =====================>>>>  保存js文件   <<<=================================
                    // TODO 保证key的顺序一致性
                    let saveFileFullPath = path.join(this.jsSavePath, jsFileName + ".ts");
                    let saveStr = "window['XMLData'] = window['XMLData'] || {};\nwindow['XMLData'][\'" + jsFileName + "\'] = " + JSON.stringify(jsSaveDataOne);
                    // let saveStr = "module.exports = " + JSON.stringify(jsSaveDataOne) + ";";
                    // if (false) {// 保存为格式化代码this.isFormatJsCode
                    ...
                    //         this._addLog("[JavaScript]" + ret.code);
                    //     } else {
