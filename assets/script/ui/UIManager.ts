...
export default class UIManager extends cc.Component {
    ...
    public async enterMain(url: string): Promise<cc.Node | null> {
        ...
        this.hideLoading();
        if (!prefab) {
            cc.error(`[UIManager.enterMain] can not find prefab: ${url}`);
            return null;
        }
    ...
    public openDialog(url: string, ...args: any[]): void {
        ...
        if (!prefab) {
            cc.error(`[UIManager.openDialog] can not find dialog prefab: ${url}`);
            return;
        }
    ...
    public async openDialogAsync(url: string, ...args: any[]): Promise<void> {
        ...
        this.hideLoading();
        if (!prefab) {
            cc.error(`[UIManager.openDialogAsync] can not find dialog prefab: ${url}`);
            return;
        }
    ...
    public async showTip(data: TipData | string): Promise<void> {
        ...
        if (this._tipPool.length > 0) {
            ...
            if (!prefab) {
                cc.error(`[UIManager.showTip] can not load prefab: ${ResUrl.PREFAB.TIP}`);
                return;
            }
