...
export default class RecyclePool {
    /** 以url标记的节点池 */
    private _urlMap: Map<string, cc.Node[]> = new Map();
    /** 以cccomponent标记的节点池，需要实现接口RecycleNode */
    private _cmptMap: Map<{ prototype: cc.Component }, cc.Node[]> = new Map();

    /** 单个节点池的最大节点数量 */
    public limit: number = 512;

    /**
     * 获取节点池中节点数量
     */
    public size(key: string | { prototype: cc.Component }): number {
     ...
     * 清空节点
     */
    public clear(key: string | { prototype: cc.Component }): void {
     ...
     * 根据类型从节点池取出节点
     */
    public get(key: string | { prototype: cc.Component }): cc.Node {
        if (typeof key === "string") {
            let list = this._urlMap.get(key);
     ...
     * 根据类型将节点放入节点池
     */
    public put(key: string | { prototype: cc.Component }, node: cc.Node): void {
        if (!node) {
